...
**Сделано:**
- **Продвижение места (задача 1):**
  - `Place.is_promoted` (BooleanField, default False) + миграция `places.0003_alter_place_options_place_is_promoted` (плюс Meta.ordering → `["-is_promoted", "name_ru"]`).
  - Админка: `is_promoted` в `list_display`/`list_editable`/`list_filter` и в fieldsets «Модерация» (включается вручную админом).
  - API `/api/places/`: стабильная сортировка продвинутых первыми — `sort(key=lambda p: (not p.is_promoted, p.distance_km))` (при координатах), иначе порядок из Meta-ordered queryset.
  - `is_promoted` добавлен в `PlaceListSerializer` и `PlaceDetailSerializer`.
  - UI: бейдж «★ Продвинутое» (`place-card__badge`) в карточке места; ключ `promoted` в `js_strings`.
- **Донаты (задача 2):**
  - Страница `/donate/` (view `places.views.donate`, URL `donate`), шаблон `frontend/templates/donate.html` (extends base.html, page.css; текст «Почему донаты важны», кнопка «Сделать донат ❤» на `DONATE_URL` из env, заглушка если не задана, ссылка «Вернуться на карту»).
  - `settings.DONATE_URL = os.environ.get('DONATE_URL', '')`, добавлен в `.env.example`.
  - Ссылка «Поддержать» (`{% url 'donate' %}`) в навигации `base.html`; ключ `donate_btn` в `js_strings` (переведён для будущего использования в JS).
- Переводы: новых строк — Продвинутое, Поддержать проект, Поддержать, Почему донаты важны, Сделать донат, «Ссылка для донатов появится в ближайшее время», Вернуться на карту + 2 длинных абзаца donate.html (ru — identity, en/tg переведены), сняты fuzzy-флаги («Сделать донат»); `compilemessages`.
- Проверено тест-клиентом (sqlite): промо-место на 46 км выведено первым перед близкими (3.4/7.3 км) — сортировка работает; `is_promoted` есть в списке и детали; `/donate/` = 200, на en содержит «Support»; навигация содержит ссылку на `/donate/`; home содержит `promoted`/`donate_btn` в js_strings. `manage.py check` (0 silenced), `makemigrations --check` без изменений. Тестовые места после проверки удалены.

**Не сделано / отложено:**
- Живой рендер бейджа/страницы донатов в браузере не проверялся (нет браузера).
- Настоящий `DONATE_URL` (платёжная ссылка/реквизиты) — заполнит основатель; пока на странице заглушка.
- Онлайн-оплата донатов/продвижения — вне MVP (PROJECT.md раздел 13).

**Заметки для следующей сессии:**
- Этап 7 закрыт (2/2). **Все этапы MVP завершены** — остался только блочер Этапа 0 «PostGIS на машине основателя/Neon» из ISSUES.md.
- Коммит Этапа 7 — дописать хеш в конце сессии.
- Каскадные проверки перед коммитом: `manage.py check`, `makemigrations --check --dry-run`, тест-клиент — как обычно sqlite (`DATABASE_URL=`) в песочнице.
